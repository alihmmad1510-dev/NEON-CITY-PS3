...
#  DESCRIPTION: Generate PKG images to reach ~3MB size
# =====================================================================
...
def draw_icon0(path):
    """512x288 main XMB icon."""
    W, H = 512, 288
    img = Image.new('RGB', (W, H), DARK_BG)
    d = ImageDraw.Draw(img)

    _sky_gradient(d, W, H,
        (8, 20, 45), (60, 100, 160), (150, 190, 220), H*3//5)
    _draw_grid(d, W, H, H*3//5)

    d.polygon([(W//2-40, H*3//5), (W//2+40, H*3//5), (W, H), (0, H)],
              fill=(34, 36, 44))
    ...
    for i in range(6):
        ...
        y1 = y0 + 20
        w0 = 4 + i
        d.polygon([(W//2-w0, y0), (W//2+w0, y0),
                   (W//2+w0, y1), (W//2-w0, y1)], fill=(255, 204, 0))
    ...
    d.text((W//2 - 95, 55), "U L T R A   v3", fill=NEON_ORANGE)

    img.save(path, "PNG")
    print(f"ICON0.PNG created: {path} ({os.path.getsize(path)} bytes)")


def draw_pic0(path):
    """1920x1080 day scene background."""
    W, H = 1920, 1080
    img = Image.new('RGB', (W, H), DARK_BG)
    d = ImageDraw.Draw(img)
    horizon = 620
    ...
    _sky_gradient(d, W, H,
        (5, 18, 40), (80, 130, 180), (200, 220, 240), horizon)
    ...
    d.polygon([(W//2-180, horizon), (W//2+180, horizon), (W, H), (0, H)],
              fill=(30, 32, 40))
    for i in range(8):
        y0 = horizon + 40 + i*70
        y1 = y0 + 40
    ...
    d.text((W//2 - 200, 200), "U L T R A   v 3", fill=NEON_ORANGE)
    d.text((W//2 - 100, 260), "3D 360 DRIVE", fill=(255,255,255))

    img.save(path, "PNG")
    print(f"PIC0.PNG created: {path} ({os.path.getsize(path)} bytes)")


def draw_pic1(path):
    """1920x1080 night neon background."""
    W, H = 1920, 1080
    ...
    d.text((W//2 - 200, 200), "U L T R A   v 3", fill=NEON_CYAN)
    d.text((W//2 - 100, 260), "NIGHT DRIVE", fill=(255,255,255))

    img.save(path, "PNG")
    print(f"PIC1.PNG created: {path} ({os.path.getsize(path)} bytes)")


def draw_preview(path, idx, W=1280, H=720):
    """Generate preview screenshots — each ~200-400KB."""
    img = Image.new('RGB', (W, H), DARK_BG)
    d = ImageDraw.Draw(img)
    horizon = H * 3 // 5
    ...
    d.polygon([(W//2-120, horizon), (W//2+120, horizon), (W, H), (0, H)],
              fill=(28, 30, 38))
    ...
    for i in range(10):
        ...
        y1 = y0 + 25
        w = 4 + i//2
        d.polygon([(W//2-w, y0), (W//2+w, y0),
                   (W//2+w, y1), (W//2-w, y1)], fill=(255, 204, 0))
    ...
    for _ in range(6):
        ...
        by = horizon - random.randint(80, 200)
        bw = random.randint(120, 220)
        d.rectangle([bx, by, bx+bw, horizon], fill=(60, 65, 90))
        for wy in range(by+10, horizon-10, 20):
            for wx in range(bx+8, bx+bw-8, 18):
                if random.random() < 0.4:
                    d.rectangle([wx, wy, wx+8, wy+12], fill=(255, 238, 136))

    cx, cy = W//2, H - 100
    d.ellipse([cx-100, cy+20, cx+100, cy+45], fill=(0,0,0))
    d.rectangle([cx-100, cy-15, cx+100, cy+30], fill=(232, 184, 36))
    d.rectangle([cx-65, cy-50, cx+65, cy-15], fill=(232, 184, 36))
    d.rectangle([cx-58, cy-45, cx-10, cy-18], fill=(48, 64, 80))
    d.rectangle([cx+10, cy-45, cx+58, cy-18], fill=(48, 64, 80))
    d.rectangle([cx-100, cy-3, cx+100, cy+3], fill=(255,255,255))
    d.ellipse([cx-80, cy+15, cx-50, cy+45], fill=(0,0,0))
    d.ellipse([cx+50, cy+15, cx+80, cy+45], fill=(0,0,0))

    d.text((30, 30), "NEON CITY", fill=NEON_CYAN)
    d.text((30, 60), "SCREENSHOT", fill=NEON_ORANGE)
    d.text((30, H-60), f"SPEED {80 + idx*5} KM/H", fill=(255, 204, 0))

    d.rectangle([W-150, 30, W-30, 90], fill=(0, 0, 0))
    d.text((W-140, 45), f"PREVIEW {idx+1}", fill=NEON_CYAN)
...
if __name__ == '__main__':
    ...
    print(f"=== Total PNG size: {total/1024/1024:.2f} MB ===")
