...
def gen_texture(idx):
    ...
    if k == 0:
        ...
        m = (xx<2)|(yy<2)
        r = np.where(m, 60, 165+(bx*11+by*7)%40)
        g = np.where(m, 60, 85+(bx*13+by*3)%30)
        b = np.where(m, 60, 65+(bx*17+by*5)%20)
    elif k == 2:
        ...
        w = np.sin(y/30)*20+np.sin(x/50)*10
        n = np.random.randn(H,W)*10
        r = 125+w+n; g = 72+w*0.5+n; b = 42+n
    elif k == 5:
        grad = (x/W)*50
        ...
        xx = x%64; yy = y%64
        m = (xx<2)|(yy<2)
        r = np.where(m, 100, 185+(tx*7+ty*3)%40)
        g = np.where(m, 100, 185+(tx*11+ty*5)%40)
        b = np.where(m, 100, 195+(tx*13+ty*17)%40)
    elif k == 8:
        n = np.random.randn(H,W)*35
        s = np.sin(x/15)*np.cos(y/12)
        r = 135+n+s*45; g = 75+n*0.5+s*22; b = 45+n*0.3
    else:
